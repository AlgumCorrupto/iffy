defmodule Iffy.Fetcher do
  ...
  def do_fetch, do:
    fetch_news_articles() |> parse_html |> compare_and_write_to_disk
  ...
  defp parse_html(html) do
    ...
    for article <- parsed_doc do
      %{
        "desc" => article |> Floki.find("span.description")
        |> Floki.text,
        "img" => article |> Floki.find("img.tileImage")
        |> Floki.attribute("src") |> list_or_nil,
        "url" => article |> Floki.find("a.summary")
        |> Floki.attribute("href") |> hd,
        "title" => article |> Floki.find("a.summary")
        |> Floki.text,
        "topic" => article |> Floki.find("span.subtitle")
        |> Floki.text,
      }
    end
  end

  defp compare_and_write_to_disk(articles) do
    ...
    if articles != new_articles do new_articles
    else [] end
  end
  ...
  defp get_til_good, do:
    Finch.build(:get, 
      "https://portal1.iff.edu.br/o-iffluminense/noticias/ultimas-noticias",
      [{"User-Agent", "villaca.p@gsuite.iff.edu.br"}]
    )
    |> Finch.request(:fetcher) |> get_til_good
  defp get_til_good({:error, _}), do:
    get_til_good()
  defp get_til_good({:ok, response}), do:
    response

  defp get_last_articles({:ok, contents}), do:
